...
function silentScore(data) {
  // Sinais válidos de falha silenciosa — apenas evidências concretas de mau funcionamento
  // REMOVIDOS: status_congelado e connector_status_frozen — geram falsos positivos
  // em estações paradas legitimamente (Available por horas = comportamento normal)
  let score = 0
  const reasons = []

  // Sessão ativa mas medidor não registra energia — evidência forte de falha
  if (data.activeTxMeterFrozen)
    { score += 3; reasons.push("meter_values_frozen") }

  // Alta taxa de cargas zeradas — mín 3 sessões para evitar ruído
  if (data.abortRate > 0.5 && data.recentTxCount >= 3)
    { score += 2; reasons.push("high_abort_rate") }

  // RemoteStart rejeitado pelo equipamento
  if (data.remoteStartRejected)
    { score += 3; reasons.push("remote_start_rejected") }

  // Múltiplos disconnect 1006 — queda repetida de conexão OCPP
  if ((data.disconnect1006Count ?? 0) > 3)
    { score += 2; reasons.push("disconnect_1006_repeated") }

  // Emergency Stop — sinal físico crítico
  if (data.emergencyStop)
    { score += 5; reasons.push("emergency_stop") }

  // Porta aberta — verificação presencial necessária
  if (data.portaAberta)
...
export function classify(raw) {
  ...
  const mm = !raw._skipFixed && MANUAL_MAINTENANCES[raw.stationID]
  if (mm) return {
    status: "maintenance",
  ...
  // Sem nenhum log (heartbeat OU StatusNotification) há mais de 6 minutos
  // = estação parou de se comunicar mas ainda não caiu o heartbeat
  const logSilenceMs = raw.lastConnectorChange
    ? now - raw.lastConnectorChange
    : (raw.lastHeartbeat ? now - raw.lastHeartbeat : Infinity)
  if (logSilenceMs > T.STATUS_FROZEN_MS) return {
    status: "error",
    statusSince: now - logSilenceMs,
    silentScore: score + 2,
    silentReasons: [...reasons, "sem_log_6min"],
  }
...
export async function buildStationList(connected, nameMap, ocppMap, bffTxMap, ocppTxMap, kv) {
  // Carrega fixas encerradas do Redis
  let endedFixas = new Set()
  if (kv) {
    try {
      const raw = await kv.get("manutencoes:fixas:encerradas")
      if (raw) endedFixas = new Set(JSON.parse(raw))
    } catch {}
  }
  ...
  for (const s of connected) {
    ...
    // Se é fixa encerrada, reclassifica normalmente em vez de maintenance
    if (endedFixas.has(sid)) {
      const classification = classify({...raw, _skipFixed: true}) ?? {
        status: "unknown", statusSince: Date.now(), silentScore: 0, silentReasons: [],
      }
      result.push({...raw, ...classification})
      continue
    }
